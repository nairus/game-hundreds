$(document).ready(function() {
   ...
   */
  functions.getBestScores = () => {
    let bestScores = [];
    if (window.localStorage) {
      bestScores = window.localStorage.getItem(game.models.BEST_SCORES) || [];
      if ("string" === typeof bestScores) {
  ...
  /**
   * Ordonne les meilleurs scores.
   *
   * @param {Array} bestScores Liste des meilleurs scores.
   */
  functions.sortBestScores = bestScores => {
    bestScores.sort((a, b) => {
      if (a.score > b.score) {
        return 1;
  ...
  /**
   * Reset la liste des meilleurs scores.
   */
  functions.resetBestScores = () => {
    game.models.bestScores = [];
    window.localStorage.removeItem(game.models.BEST_SCORES);
  };

  /**
   * Sauvegarde le score du joueur.
   *
   * @param {strint} player Le nom du joueur.
   * @param {number} score  Le score (temps passé).
   */
  functions.saveScore = (player, score) => {
    //on récupère les meilleurs scores.
    let bestScores = game.models.bestScores;

    // on ajoute le score.
    bestScores.push({ pseudo: player, score: score, date: Date.now() });

    // on ordonne la liste
    bestScores = functions.sortBestScores(bestScores);

    // on supprime le dernier score si + de 10 scores sauvés
